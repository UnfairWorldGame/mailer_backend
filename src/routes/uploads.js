...
async function importContacts(contacts, sourceLabel, stats = {}) {
  ...
  const { inserted, updated, total } = await upsertContacts(contacts);
...
router.get('/history', async (req, res, next) => {
  try {
    ...
    const history = await UploadHistory.find().sort({ created_at: -1 }).limit(limit);
...
router.get('/contacts/count', async (_req, res, next) => {
  try {
    res.json(await getContactMeta());
  } catch (err) {
    next(err);
  }
});
...
router.delete('/contacts/:id', async (req, res, next) => {
  try {
    ...
    res.json({ message: 'Contact deleted', ...(await getContactMeta()) });
  } catch (err) {
    next(err);
  }
});
...
router.post('/emails', async (req, res, next) => {
  try {
    ...
    const result = await importContacts(parsed.contacts, 'pasted-emails.txt', parsed.stats);
    if (result.error) {
...
router.post('/excel', upload.single('file'), async (req, res, next) => {
  ...
  try {
    ...
    const result = await importContacts(contacts, req.file.originalname, stats);
    if (result.error) {
      return res.status(result.status).json({ error: result.error });
    }
    res.json(result.body);
